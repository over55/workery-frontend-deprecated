...
import {
    WORKERY_CLIENT_FILE_LIST_API_ENDPOINT,
    WORKERY_CLIENT_FILE_DETAIL_API_ENDPOINT,
    WORKERY_CLIENT_FILE_ARCHIVE_API_ENDPOINT
} from '../constants/api';
...
export function pullClientFileUploadList(page=1, sizePerPage=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
        ...
        let aURL = WORKERY_CLIENT_FILE_LIST_API_ENDPOINT+"?page="+page+"&page_size="+sizePerPage;
        filtersMap.forEach(
            (value, key) => {
...
export function postClientFileUpload(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        customAxios.post(WORKERY_CLIENT_FILE_LIST_API_ENDPOINT, decamelizedData).then( (successResponse) => {
            const responseData = successResponse.data;
...
export function deleteClientFileUpload(id, successCallback, failedCallback) {
    return dispatch => {
        ...
        const aURL = WORKERY_CLIENT_FILE_ARCHIVE_API_ENDPOINT.replace("XXX", id);
