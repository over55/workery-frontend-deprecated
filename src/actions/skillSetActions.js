...
export function pullSkillSetList(offset=0, limit=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
        ...
        let aURL = WORKERY_SKILL_SET_LIST_API_ENDPOINT+"?offset="+offset+"&page_size="+limit;
        filtersMap.forEach(
            (value, key) => {
...
export function postSkillSetDetail(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        customAxios.post(WORKERY_SKILL_SET_LIST_API_ENDPOINT, decamelizedData).then( (successResponse) => {
            const responseData = successResponse.data;
