...
export function pullHowHearDetail(user, slug) {
    return dispatch => {
        // Change the global state to attempting to fetch latest user details.
        store.dispatch(
            setHowHearDetailRequest()
        ...
        const aURL = WORKERY_HOW_HEAR_DETAIL_API_ENDPOINT+slug;
...
export function putHowHearDetail(user, data, successCallback, failedCallback) {
    return dispatch => {
        // Change the global state to attempting to log in.
        store.dispatch(
            setHowHearDetailRequest()
        ...
        // Perform our API submission.
        customAxios.put(WORKERY_HOW_HEAR_DETAIL_API_ENDPOINT+data.slug, buffer).then( (successResponse) => {
            // Decode our MessagePack (Buffer) into JS Object.
            const responseData = msgpack.decode(Buffer(successResponse.data));
            let device = camelizeKeys(responseData);
