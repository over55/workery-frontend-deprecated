...
export function pullStaffList(offset=1, limit=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
...
export function putStaffChangeRoleOperation(postData, onSuccessCallback, onFailureCallback) {
    return dispatch => {
        ...
        // Perform our API submission.
        customAxios.put(WORKERY_STAFF_CHANGE_ROLE_OPERATION_API_ENDPOINT.replace("XXX", postData.id), decamelizedData).then( (successResponse) => {
            const responseData = successResponse.data;
            let staff = camelizeKeys(responseData);
