...
export function pullOrderCommentList(page=1, sizePerPage=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
        ...
        let aURL = WORKERY_ORDER_COMMENT_LIST_API_ENDPOINT+"?page="+page+"&page_size="+sizePerPage;
        filtersMap.forEach(
            (value, key) => {
...
export function postOrderComment(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        customAxios.post(WORKERY_ORDER_COMMENT_LIST_API_ENDPOINT, buffer).then( (successResponse) => {
            // Decode our MessagePack (Buffer) into JS Object.
            const responseData = msgpack.decode(Buffer(successResponse.data));
