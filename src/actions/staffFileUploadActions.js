...
import {
    WORKERY_STAFF_FILE_LIST_API_ENDPOINT,
    WORKERY_STAFF_FILE_DETAIL_API_ENDPOINT,
    WORKERY_STAFF_FILE_ARCHIVE_API_ENDPOINT
} from '../constants/api';
...
export function pullStaffFileUploadList(page=1, sizePerPage=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
        ...
        let aURL = WORKERY_STAFF_FILE_LIST_API_ENDPOINT+"?page="+page+"&page_size="+sizePerPage;
        filtersMap.forEach(
            (value, key) => {
...
export function postStaffFileUpload(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        customAxios.post(WORKERY_STAFF_FILE_LIST_API_ENDPOINT, buffer).then( (successResponse) => {
            // Decode our MessagePack (Buffer) into JS Object.
            const responseData = msgpack.decode(Buffer(successResponse.data));
...
export function deleteStaffFileUpload(id, successCallback, failedCallback) {
    return dispatch => {
        ...
        const aURL = WORKERY_STAFF_FILE_ARCHIVE_API_ENDPOINT.replace("XXX", id);
