...
import { WORKERY_PRIVATE_FILE_LIST_API_URL, WORKERY_PRIVATE_FILE_DETAIL_API_URL } from '../constants/api';
...
export function pullPrivateFileList(offset=0, limit=10, filtersMap=new Map(), onSuccessCallback=null, onFailureCallback=null) {
    return dispatch => {
        ...
        let aURL = WORKERY_PRIVATE_FILE_LIST_API_URL+"?offset="+offset+"&limit="+limit;
        filtersMap.forEach(
            (value, key) => {
...
export function postPrivateFileDetail(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        customAxios.post(WORKERY_PRIVATE_FILE_LIST_API_URL, decamelizedData).then( (successResponse) => {
            const responseData = successResponse.data;
...
export function pullPrivateFileDetail(id, onSuccessCallback, onFailureCallback) {
    return dispatch => {
        ...
        const aURL = WORKERY_PRIVATE_FILE_DETAIL_API_URL+id+"/";
...
export function putPrivateFileDetail(postData, successCallback, failedCallback) {
    return dispatch => {
        ...
        const aURL = WORKERY_PRIVATE_FILE_DETAIL_API_URL+postData.id+"/";
