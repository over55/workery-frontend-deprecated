        ...
        if (type === "sort") {
            ...
            this.setState(
                ...
                ()=>{
                    // STEP 3:
                    // SUBMIT TO OUR API.
                    this.props.pullAwayLogList(this.state.page, this.state.sizePerPage, parametersMap, this.onSuccessfulSubmissionCallback, this.onFailedSubmissionCallback);
                }
            );
            ...
            this.setState(
                ...
                ()=>{
                    this.props.pullAwayLogList(page, sizePerPage, this.state.parametersMap, this.onSuccessfulSubmissionCallback, this.onFailedSubmissionCallback);
                }
            );

        } else if (type === "filter") {
            console.log(type, filters); // For debugging purposes only.
            if (filters.wasDeleted === undefined) {
                parametersMap.delete("wasDeleted");
            } else {
                const filterVal = filters.wasDeleted.filterVal;
                parametersMap.set("wasDeleted", filterVal);
            }
            this.setState(
                { parametersMap: parametersMap, isLoading: true, },
                ()=>{
    ...
    render() {
        ...
        return (
            <AwayLogListComponent
                ...
                totalSize={totalSize}
                awayLogList={this.props.awayLogList}
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        pullAwayLogList: (page, sizePerPage, map, onSuccessCallback, onFailureCallback) => {
            dispatch(
                pullAwayLogList(page, sizePerPage, map, onSuccessCallback, onFailureCallback)
            )
        },
